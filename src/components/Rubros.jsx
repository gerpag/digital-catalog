  ...
  useEffect(() => {
    const productsFetch = async () => {
      try {
        ...
        const urlsImages = response.data.map((url) => url.url_img);
  ...
  useEffect(() => {
    const updatedFilteredProducts = products.filter((product) => {
      ...
      return searchTermsArray.every((term) => {
        ...
        const productColour = typeof product.colour === "string" ? product.colour.toLowerCase() : "";
        return productName.includes(term) || productColour.includes(term);
      });
    });
